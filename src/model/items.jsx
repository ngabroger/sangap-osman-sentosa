import acrylic from '../assets/images/acrylic.jpg';
import extreme from '../assets/images/extreme.jpg';
import juntax from '../assets/images/juntax.jpg';

import metalpower from '../assets/images/metalpower.jpg';
import foxxy from '../assets/images/foxxy.jpg';
var Link = 'https://sangaposmansentosa.vercel.app';
export const items = [
  {
    id: 1,
    image: Link + '/assets/extreme-DDSKyYzx.jpg ',
    title: 'Extreme Waterproof',
    description:
      ' Pelapis anti bocor berkualitas tinggi untuk berbagai permukaan, memberikan perlindungan jangka panjang terhadap kebocoran.',
    advantage: (
      <>
        Memiliki daya rekat yang kuat pada berbagai jenis permukaan.
        <br />
        Tahan terhadap tekanan air tinggi, cocok untuk area basah.
        <br />
        Fleksibel, tidak mudah retak meskipun permukaan bergerak.
        <br />
        Mudah diaplikasikan dengan kuas atau roller.
        <br />
        Cocok untuk penggunaan di atap, dinding, atau kolam.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
  {
    id: 2,
    image: Link + '/assets/acrylic-BdHIYhaa.jpg',
    title: 'Acrylic SOS',
    description:
      '  Bahan tambahan khusus untuk meningkatkan performa cat pada berbagai kondisi lingkungan.',
    advantage: (
      <>
        Meningkatkan ketahanan cat terhadap jamur dan lumut.
        <br />
        Melindungi dari pemudaran warna akibat sinar UV.
        <br />
        Memperpanjang daya tahan cat pada kondisi lembap atau panas.
        <br />
        Mudah dicampurkan tanpa mengubah sifat cat.
        <br />
        Meningkatkan hasil akhir cat menjadi lebih halus dan tahan lama.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
  {
    id: 3,
    image: Link + '/assets/metalpower-D3m0LZJ9.jpg',
    title: 'Metal Power',
    description:
      'Cat pelapis premium yang dirancang untuk melindungi dan memperindah permukaan besi dan kayu tanpa memerlukan thinner.',
    advantage: (
      <>
        Memberikan perlindungan maksimal terhadap karat pada besi dan rayap pada
        kayu.
        <br />
        Cepat kering dengan hasil akhir mengkilap dan tahan lama.
        <br />
        Tahan terhadap cuaca ekstrem dan kelembapan tinggi.
        <br />
        Mudah diaplikasikan langsung tanpa perlu bahan tambahan.
        <br />
        Cocok untuk berbagai proyek interior maupun eksterior.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
  {
    id: 4,
    image: Link + '/assets/juntax-BBdDXD0T.jpg',
    title: 'Juntax Paint',
    description:
      '  Cat tembok berkualitas tinggi dengan harga ekonomis,tersedia dalam berbagai ukuran termasuk galon dan drum 200 kg.',
    advantage: (
      <>
        Tahan terhadap cuaca ekstrem dan kelembapan tinggi.
        <br />
        Mudah diaplikasikan langsung tanpa perlu bahan tambahan.
        <br />
        Cocok untuk berbagai proyek interior maupun eksterior.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
  {
    id: 5,
    image: Link + '/assets/foxxy-BAPtQghl.jpg',
    title: 'Lem Foxxy',
    description:
      '   Lem putih serbaguna yang ideal untuk berbagai aplikasi seperti kayu, kertas, dan kerajinan tangan.',
    advantage: (
      <>
        Cepat kering, menghemat waktu pengerjaan.
        <br />
        Memberikan daya rekat yang kuat dan tahan lama.
        <br />
        Tidak berbau menyengat, aman untuk penggunaan di dalam ruangan.
        <br />
        Cocok untuk berbagai kebutuhan, baik profesional maupun DIY.
        <br />
        Aman dan ramah lingkungan.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
  {
    id: 6,
    image: Link + '/assets/viotex.jpeg ',
    title: 'Viotex Premium',
    description:
      ' Cat elastomer eksterior berkualitas tinggi yang dirancang untuk menutup retak rambut dan memberikan perlindungan kedap air.',
    advantage: (
      <>
        2X UV Protected Colours untuk ketahanan warna lebih lama.
        <br />
        Menutup dan mencegah keretakan pada dinding.
        <br />
        Waterproof, tahan terhadap rembesan air.
        <br />
        Waktu kering 4 jam dengan hasil akhir sheen finish.
        <br />
        Daya sebar hingga 13 m2 per lapis.
      </>
    ),
    link: 'https://www.tokopedia.com/catosman/cat-besi-dan-kayu-tanpa-thiner-anti-karat-metal-power-1kg-1730832064224200185',
  },
];
